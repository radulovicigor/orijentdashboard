...
function dailySpendByEntity(rows: Row[], kind: "campaign", since: string, until: string): Map<string, { date: string; spend: number }[]> {
  const days = eachDay(since, until);
  const out = new Map<string, Map<string, number>>();
  for (const r of rows) {
    if (r.kind !== kind || !inRange(r.date, since, until)) continue;
    const m = out.get(r.entity_id) ?? new Map<string, number>();
...
export async function getMetaData(key: RangeKey, since: string, until: string, prev: { since: string; until: string }): Promise<MetaData> {
  ...
  const campaignDaily = dailySpendByEntity(rows, "campaign", since, until);

  const allAds: MetaAd[] = group("ad")
    .map(([id, g]) => ({
      id,
      name: g.last.name ?? id,
